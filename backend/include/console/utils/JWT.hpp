...
class JWT {
  ...
  private:
    ...
    static jwt::builder<jwt::default_clock, json_traits> create_token_builder(const JWTClaims& claims);

    /**
     * @brief Extract claims from decoded JWT
     */
    static JWTClaims extract_claims(const jwt::decoded_jwt<json_traits>& decoded);
