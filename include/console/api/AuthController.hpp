...
class AuthController : public drogon::HttpController<AuthController> {
  public:
    ...
    // Get current user
    ADD_METHOD_TO(AuthController::me, "/api/v1/auth/me", drogon::Get, "console::middleware::AuthMiddleware");
    ...
    // Change password
    ADD_METHOD_TO(AuthController::change_password,
  ...
  private:
    String generate_jwt_token(const UserInfo& user) const;
    Optional<UserInfo> validate_credentials(const String& username, const String& password) const;
    Optional<UserInfo> validate_token(const String& token) const;
};
