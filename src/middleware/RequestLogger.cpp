...
void
RequestLogger::doFilter(const drogon::HttpRequestPtr& req,
    ...
    auto logging_callback = [this, req, start_time, fcb = std::move(fcb)](const drogon::HttpResponsePtr& resp) mutable {
    ...
    CONSOLE_LOG_DEBUG("Request started: {} {} from {}", req->getMethodString(), req->getPath(), get_client_ip(req));
}

String
RequestLogger::format_request_log(const drogon::HttpRequestPtr& req, std::chrono::milliseconds duration) const {
    std::ostringstream oss;

    oss << req->getMethodString() << " " << req->getPath();
    ...
    oss << " - Duration: " << duration.count() << "ms"
        << " - IP: " << get_client_ip(req) << " - Size: " << get_request_size(req) << " bytes";

    return oss.str();
}

String
RequestLogger::get_client_ip(const drogon::HttpRequestPtr& req) const {
...
size_t
RequestLogger::get_request_size(const drogon::HttpRequestPtr& req) const {
    size_t size = 0;

    // Add method line
    size += std::strlen(req->getMethodString()) + 1; // Method + space
